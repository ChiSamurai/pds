...
/** Optionally provides the initial set of {@link NavEntry}s that's pushed to the {@link NavEntries} state */
export const NAV_ENTRIES = new InjectionToken<NavEntry[]>('NAV_ENTRIES');

@Injectable({ providedIn: 'root' })
export class NavEntries extends ArrayBehaviorState<NavEntry> {
  constructor(@Optional() @Inject(NAV_ENTRIES) initialEntries?: /* @dynamic */ NavEntry[]) {
    super();
