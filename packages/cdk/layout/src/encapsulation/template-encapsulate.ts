...
function resolveEncapsulateTemplate(dir: TemplateEncapsulate): TemplateRef<any> {
  return dir.template;
}
...
export class TemplateEncapsulate implements DoCheck, OnDestroy {
  ...
  constructor(
    ...
    @Optional() @Inject(TEMPLATE_ENCAPSULATIONS)
  ...
  protected destroyContainer(): void {
    if (this.container != null) {
      if (this.container instanceof ComponentRef && this.container.hostView.destroyed
  ...
  ngDoCheck() {
    if (this.encapsulation != null) {
      ...
      this.container = this.encapsulation.container instanceof TemplateRef
