...
export interface ControlCloneOptions<T extends AbstractControl = AbstractControl> {
  fallbackFactory?: (control: NonNullable<T>) => T;
  preserveValue?: boolean;
}

export function cloneAbstractControl<T extends AbstractControl>(control: NonNullable<T>, options?: ControlCloneOptions<T>): T {
  let newControl: T;
  ...
  } else if (options != null && typeof options.fallbackFactory === "function") {
    return options.fallbackFactory(control);
  } else {
